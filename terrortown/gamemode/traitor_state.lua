...
function SendConfirmedKillers(ply_or_rf)
	SendKillerList(ply_or_rf, function(p) return p:GetNWBool("body_found") end)
end
...
local function request_rolelist(ply)
	...
	if GetRoundState() != ROUND_WAIT then
		...
		if ply:IsTraitor() or ply:IsKiller() then
			SendTraitorList(ply)
		else
...
concommand.Add("ttt_force_killer", force_killer)
