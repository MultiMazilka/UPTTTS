...
local function KillerSort(a,b)
	if not IsValid(a) then return true end
	if not IsValid(b) then return false end
	
	if a:GetKiller() and (not b:GetKiller()) then return true end
	if b:GetKiller() and (not a:GetKiller()) then return false end
...
function PrintKillers(ply)
	if not IsValid(ply) or ply:IsSuperAdmin() then
		ServerLog(Format("%s used ttt_print_killers\n", IsValid(ply) and ply:Nick() or "console"))
		
		local pr = GetPrintFn(ply)
		
		local ps = player.GetAll()
		table.sort(ps, KillerSort)
		
		for _, p in pairs(ps) do
			if IsValid(p) then
				pr(p:GetKiller() and "KILLER" or "Innocent", ":", p:Nick())
			end
		end
	end
end
concommand.Add("ttt_print_killers", PrintKillers)
