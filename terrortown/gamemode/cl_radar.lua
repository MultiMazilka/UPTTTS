...
function RADAR:Timeout()
	...
	if self.repeating and LocalPlayer() and (LocalPlayer():IsActiveTraitor() or LocalPlayer():IsActiveDetective() or LocalPlayer():IsActiveKiller()) then
...
function RADAR:Draw(client)
	...
	for k, tgt in pairs(RADAR.targets) do
		...
		if role == ROLE_TRAITOR then
			...
			surface.SetDrawColor(255,138,20, alpha)
			surface.SetTextColor(255,138,20, alpha)
