...
AddCSLuaFile("cl_traitorglows.lua")
...
CreateConVar("ttt_killer_pct", "0.7")
CreateConVar("ttt_killer_max", "1")
CreateConVar("ttt_kill_credits_starting", "3")
...
function PrintResultMessage(type)
	...
	if type == WIN_TIMELIMIT then
		...
		LANG.Msg("win_killers")
		ServerLog("Result: killer win.\n")
...
function GM:TTTCheckForWin()
	...
	if GAMEMODE.MapWin == WIN_TRAITOR or GAMEMODE.MapWin == WIN_INNOCENT or GAMEMODE.MapWin == WIN_KILLER then
	...
	for k,v in pairs(player.GetAll()) do
		...
		if traitor_alive and innocent_alive and killer_alive then
...
local function GetKillerCount(ply_count)
	-- get number of killers: pct of players rounded down
	local killer_count = math.floor(ply_count * GetConVar("ttt_killer_pct"):GetFloat())
	-- make sure there is at least 1 killer
	killer_count = math.Clamp(killer_count, 1, GetConVar("ttt_killer_max"):GetInt())
...
function SelectRoles()
	...
	local killer_count = GetKillerCount(choice_count)
	...
	while (ds < det_count) and (#choices >= 1) do
		...
		local ks = 0
		while (ks < killer_count) and (#choices >= 1) do
			-- select random index in choices table
			local pick = math.random(1, #choices)
			...
			-- a roll
			if IsValid(pply) and
				((not table.HasValue(prev_roles[ROLE_KILLER], pply)) or (math.random(1, 3) == 2)) then
